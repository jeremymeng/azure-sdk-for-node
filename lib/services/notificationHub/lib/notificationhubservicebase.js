...
var NotificationHubServiceClient = require('./notificationhubserviceclient');
...
var NotificationHubSettings = azureCommon.NotificationHubSettings;

/**
* Creates a new NotificationHubServiceBase object.
*
* @constructor
...
*/
function NotificationHubServiceBase(configOrNamespaceOrConnectionStringOrSettings, accessKey, issuer, acsNamespace, host, authenticationProvider) {
  var notificationHubSettings;
  if(!configOrNamespaceOrConnectionStringOrSettings || _.isFunction(configOrNamespaceOrConnectionStringOrSettings)) {
    ...
    notificationHubSettings = NotificationHubSettings.createFromConfig(configOrNamespaceOrConnectionStringOrSettings);
  } else if (_.isObject(configOrNamespaceOrConnectionStringOrSettings)) {
    notificationHubSettings = configOrNamespaceOrConnectionStringOrSettings;
    ...
    notificationHubSettings = NotificationHubSettings.createFromConnectionString(configOrNamespaceOrConnectionStringOrSettings);
  } else {
    if (!configOrNamespaceOrConnectionStringOrSettings) {
      configOrNamespaceOrConnectionStringOrSettings = process.env[ServiceClientConstants.EnvironmentVariables.AZURE_NOTIFICATIONHUB_NAMESPACE];
    }

    if (!accessKey) {
      accessKey = process.env[ServiceClientConstants.EnvironmentVariables.AZURE_NOTIFICATIONHUB_ACCESS_KEY];
    }

    if (!issuer) {
      issuer = process.env[ServiceClientConstants.EnvironmentVariables.AZURE_NOTIFICATIONHUB_ISSUER];

      if (!issuer) {
        issuer = ServiceClientConstants.DEFAULT_NOTIFICATIONHUB_ISSUER;
      }
    }
    ...
    var endpoint = url.format({ protocol: 'https:', port: 443, hostname: configOrNamespaceOrConnectionStringOrSettings + '.' + ServiceClientConstants.CLOUD_NOTIFICATIONHUBS_HOST });
    ...
    notificationHubSettings = NotificationHubSettings.createFromSettings(settings);
  }

  NotificationHubServiceBase['super_'].call(this,
    notificationHubSettings._wrapPassword,
    notificationHubSettings._wrapName,
    notificationHubSettings._sharedAccessKeyName,
    notificationHubSettings._sharedAccessKey,
    notificationHubSettings._serviceBusEndpointUri,
    notificationHubSettings._wrapEndpointUri,
    authenticationProvider);

  this.xml2jsSettings.ignoreAttrs = true;
}

util.inherits(NotificationHubServiceBase,NotificationHubServiceClient);
...
* @return {undefined}
*/
NotificationHubServiceBase.prototype._createResource = function (path, handler, validators, optionsOrCallback, callback) {
  var options = null;
...
* @return {undefined}
*/
NotificationHubServiceBase.prototype._getResource = function (path, handler, validators, callback) {
  validateCallback(callback);
...
* @return {undefined}
*/
NotificationHubServiceBase.prototype._listResources = function (path, handler, validators, optionsOrCallback, callback) {
  var options = null;
  if (typeof optionsOrCallback === 'function' && !callback) {
    callback = optionsOrCallback;
  } else {
...
* @return {undefined}
*/
NotificationHubServiceBase.prototype._deleteResource = function (path, callback) {
  validateCallback(callback);
...
* @return {undefined}
*/
NotificationHubServiceBase.prototype._executeRequest = function (webResource, payload, resultHandler, validators, callback) {
...
module.exports = NotificationHubServiceBase;
